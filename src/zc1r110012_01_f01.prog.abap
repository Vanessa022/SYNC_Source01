...
*     AND c~spras = sy-langu
  ...
  LOOP AT gt_mast INTO gs_mast.
    ...
    CALL METHOD gcl_maktx->get_mat_description
      EXPORTING
        pi_matnr = gs_mast-matnr
      IMPORTING
        pe_maktx = gs_mast-maktx
        pe_code  = lv_code
        pe_msg   = lv_msg.

    IF lv_code EQ 'S'.

      MODIFY gt_mast FROM gs_mast INDEX lv_tabix
      TRANSPORTING maktx.
...
FORM handle_double_click  USING    ps_row    TYPE lvc_s_row
  ...
  READ TABLE gt_mast INTO gs_mast INDEX ps_row-index.
  ...
  CASE ps_column-fieldname.
    WHEN 'MATNR'.
      IF gs_mast-matnr IS INITIAL.
        EXIT.
      ENDIF.

      SET PARAMETER ID : 'MAT' FIELD gs_mast-matnr.
...
FORM handle_hotspot  USING    ps_row_id     TYPE lvc_s_row
 ...
 READ TABLE gt_mast INTO gs_mast INDEX ps_row_id-index.
 ...
 ENDIF.
  ...
  CASE ps_column_id-fieldname.
    WHEN 'STLNR'.
      IF gs_mast-stlnr IS INITIAL.
        EXIT.
      ENDIF.

      SET PARAMETER ID: 'MAT' FIELD gs_mast-matnr,
                        'WRK' FIELD pa_werks,
                        'CSV' FIELD gs_mast-stlan.
